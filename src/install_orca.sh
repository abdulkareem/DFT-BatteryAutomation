...
ORCA_URL_DEFAULT="https://orcaforum.kofo.mpg.de/app.php/dlext/?view=detail&df_id=180"
ORCA_URL="${ORCA_URL:-$ORCA_URL_DEFAULT}"
...
ensure_orca_archive() {
  ...
  if ! tar -tf "/content/downloads/${ORCA_ARCHIVE_NAME}" >/dev/null 2>&1; then
    echo "[error] Downloaded file is not a valid ORCA tar.xz archive."
    echo "[hint] ORCA forum links usually require authentication and may return HTML."
    echo "[hint] Place the ORCA tarball at: ${ORCA_LOCAL_ARCHIVE}"
